...
=============================================
Fixes applied vs v1:
  - Async job handling for large insight requests (was silently failing)
  - Delete-before-insert to prevent duplicates on every run
  - Insights pulled at account / campaign / adset / ad level separately
    (v1 only pulled at ad level — campaign/adset totals were approximations)
  - Quality rankings added (quality_ranking, engagement_rate_ranking, conversion_rate_ranking)
  - Social proof added (inline_link_clicks, inline_post_engagement)
  - Attribution setting added per ad
  - outbound_clicks extraction fixed (was grabbing wrong action_type)
  - ctr/cpc/cpm recalculated from raw totals (not stored as-is from API)
  - LOOKBACK_DAYS default reduced to 7 (was 30 — caused rate limits daily)
  - Pagination handled properly for all account discovery endpoints
  - delivery_status added to ads table
  - attribution_spec added to adsets table

Tables (19 total):
  1.  account_daily              — daily metrics at account level
  2.  campaign_daily             — daily metrics at campaign level (direct pull)
  3.  adset_daily                — daily metrics at adset level (direct pull)
  4.  ad_daily                   — daily metrics at ad level with quality rankings
  5.  ad_insights_by_country     — breakdown by country
  6.  ad_insights_by_device      — breakdown by device
  7.  ad_insights_by_placement   — breakdown by placement
  8.  ad_insights_by_age_gender  — breakdown by age + gender
  9.  campaigns                  — campaign structure + budgets
  10. adsets                     — ad set structure + targeting + attribution_spec
  11. ads                        — individual ads + delivery_status
  12. ad_creatives               — creative details
  13. ad_delivery                — quality rankings + relevance scores per ad per day
  14. page_insights              — page metrics daily
  15. pixel_events               — conversion events
  16. auction_insights           — competitive data
  17. app_events                 — app events via SDK
  18. reach_frequency            — reach & frequency per adset
  19. custom_audiences           — audience list
"""
...
# 7 for daily runs. Set to 30+ manually for initial historical load only.
...
# ─── INSIGHT FIELDS — everything visible in Facebook Ads Manager UI ───────────
INSIGHT_FIELDS = [
    ...
    # Quality rankings — shown in Facebook Ads Manager
...
SCHEMAS = {
    ...
    "ad_daily": [
        ...
        *kpi_fields(),
        ...
        bigquery.SchemaField("estimated_ad_recallers",  "INTEGER"),
        bigquery.SchemaField("estimated_ad_recall_rate","FLOAT"),
        bigquery.SchemaField("_ingested_at",            "TIMESTAMP"),
    ],
    ...
    "adsets": [
        ...
        bigquery.SchemaField("attribution_spec",                "STRING"),
        bigquery.SchemaField("start_time",                      "TIMESTAMP"),
...
def extract_outbound_clicks(insight):
    """Fixed: filter by action_type == outbound_click specifically."""
    clicks = next(
...
def build_kpi(insight):
    ...
    impressions = safe_int(insight.get("impressions")) or 0
    clicks     = safe_int(insight.get("clicks")) or 0
    outbound_clicks, outbound_ctr = extract_outbound_clicks(insight)
    ...
    return {
        ...
        # Recalculated from raw totals — never use API-provided rates
...
def load_to_bq(client, name, rows):
    ...
    # Date-based tables: delete rows for lookback window then reinsert
    date_col_map = {
        "account_daily":            "date_start",
        "campaign_daily":           "date_start",
        "adset_daily":              "date_start",
        "ad_daily":                 "date_start",
    ...
    if date_col:
        ...
        # Structure tables (campaigns, adsets, ads, creatives, audiences) — full truncate
        try:
...
def get_all_ad_accounts():
    log.info(f"Discovering all ad accounts from Business {FB_BUSINESS_ID}...")
    active   = []
    seen_ids = set()
    ...
    def process(data):
        for a in data:
            ...
            if status == 1 and act_id not in seen_ids:
                seen_ids.add(act_id)
                active.append(AdAccount(act_id))

    for endpoint in ["owned_ad_accounts", "client_ad_accounts"]:
        url    = f"https://graph.facebook.com/v18.0/{FB_BUSINESS_ID}/{endpoint}"
        params = {"fields": "id,name,account_status", "limit": 100, "access_token": FB_ACCESS_TOKEN}
        while url:
            resp   = requests.get(url, params=params).json()
            process(resp.get("data", []))
            url    = resp.get("paging", {}).get("next")
            params = {}

    log.info(f"  Total {len(active)} active ad accounts found")
    return active

# ─── ASYNC INSIGHTS FETCHER ──────────────────────────────────────────────────
def get_insights_async(account, level, breakdowns=None):
    """
    Uses async jobs — required for large date ranges.
    Synchronous calls silently return near-empty results for 30-day windows.
    """
    start, end = date_range()
    params = {
        "level":          level,
    ...
    for attempt in range(3):
        try:
            ...
            while async_job[AdReportRun.Field.async_status] != "Job Completed":
                if waited >= max_wait:
                    log.warning(f"  Async job timed out after {max_wait}s")
                    return []
                time.sleep(10)
                waited   += 10
                async_job = async_job.api_get()
                ...
                log.info(f"  Job: {status} ({pct}%) — {waited}s elapsed")
                if status in ("Job Failed", "Job Skipped"):
                    log.warning(f"  Async job failed with status: {status}")
                    return []

            results = []
            for row in async_job.get_result(params={"limit": 500}):
                results.append(row)
            log.info(f"  ✅ {len(results)} rows from async job (level={level}, account={account.get_id()})")
            return results

        except Exception as e:
            err_str = str(e)
            if any(x in err_str.lower() for x in ["rate", "too many", "limit"]):
                wait = 60 * (attempt + 1)
                log.warning(f"  Rate limit — waiting {wait}s (retry {attempt+1}/3)...")
                time.sleep(wait)
            else:
                log.warning(f"  Error (level={level}): {e}")
                return []

    log.warning("  Gave up after 3 retries")
    return []
...
def fetch_campaign_daily(accounts):
    log.info("Fetching Campaign Daily...")
    rows = []
    for account in accounts:
...
def fetch_adset_daily(accounts):
    log.info("Fetching Adset Daily...")
    rows = []
    for account in accounts:
...
def fetch_ad_daily(accounts):
    log.info("Fetching Ad Daily...")
    rows = []
    for account in accounts:
        for i in get_insights_async(account, level="ad"):
            rows.append({
                ...
                "buying_type":              i.get("buying_type"),
                **build_kpi(i),
                "quality_ranking":          i.get("quality_ranking"),
                "engagement_rate_ranking":  i.get("engagement_rate_ranking"),
                "conversion_rate_ranking":  i.get("conversion_rate_ranking"),
                "attribution_setting":      i.get("attribution_setting"),
                "estimated_ad_recallers":   safe_int(i.get("estimated_ad_recallers")),
                "estimated_ad_recall_rate": safe_float(i.get("estimated_ad_recall_rate")),
                "_ingested_at":             now_ts(),
            })
    return rows
...
def fetch_ad_delivery(accounts):
    ...
    for account in accounts:
        for i in get_insights_async(account, level="ad"):
            if any([i.get("quality_ranking"), i.get("engagement_rate_ranking"),
...
def fetch_breakdown(accounts, breakdowns, extra_keys, table_name):
    ...
    rows = []
    for account in accounts:
        for i in get_insights_async(account, level="ad", breakdowns=breakdowns):
            row = {
                "date_start":  i.get("date_start"),
                "account_id":  i.get("account_id"),
                "campaign_id": i.get("campaign_id"),
        ...
        try:
            for s in account.get_ad_sets(fields=fields, params={"limit": 500}):
                ...
                rows.append({
                    ...
                    "attribution_spec":             json.dumps(s.get("attribution_spec")) if s.get("attribution_spec") else None,
        ...
        try:
            for a in account.get_ads(fields=fields, params={"limit": 500}):
                ...
                rows.append({
                    ...
                    "delivery_status":          delivery.get("effective_status") if isinstance(delivery, dict) else None,
...
def fetch_ad_creatives(accounts):
    ...
    fields = [
        ...
        AdCreative.Field.title, AdCreative.Field.body,
...
def fetch_page_insights():
    ...
    metrics = [
        ...
        "page_video_views_unique", "page_total_actions",
    ...
    for metric in metrics:
        try:
            ...
            for m in page.get_insights(params={"metric": metric, "period": "day", "since": start, "until": end}):
...
def fetch_pixel_events(accounts):
    ...
    for account in accounts:
        try:
            ...
            log.warning(f"  Pixel events error for {account.get_id()}: {e}")
    return rows
...
def fetch_app_events(accounts):
    ...
    for account in accounts:
        try:
            for i in account.get_insights(
                fields=["date_start", "account_id", "actions"],
                params={"level": "account", "time_range": {"since": start, "until": end},
                        "time_increment": 1, "limit": 200}
            ):
...
def fetch_reach_frequency(accounts):
    ...
    for account in accounts:
        try:
            for i in account.get_insights(
                fields=[
                    AdsInsights.Field.date_start, AdsInsights.Field.account_id,
                ...
                params={"level": "adset", "time_range": {"since": start, "until": end},
        ...
        try:
            for a in account.get_custom_audiences(fields=fields, params={"limit": 200}):
                try:
                    ...
                    ds_str = json.dumps(ds.export_all_data() if hasattr(ds, "export_all_data") else (ds or {}))
                except Exception:
                    ds_str = str(a.get("data_source", ""))
                rows.append({
                    "account_id":        account.get_id(),
...
# ─── MAIN ─────────────────────────────────────────────────────────────────────
def main():
    log.info("🚀 Facebook → BigQuery sync v2")
    ...
    log.info("── Insights (account / campaign / adset / ad levels) ──")
    load_to_bq(bq, "account_daily",  fetch_account_daily(accounts))
    load_to_bq(bq, "campaign_daily", fetch_campaign_daily(accounts))
    load_to_bq(bq, "adset_daily",    fetch_adset_daily(accounts))
    load_to_bq(bq, "ad_daily",       fetch_ad_daily(accounts))
    ...
    log.info("── Auction, App Events, Reach & Audiences ──")
